...
class LOADER:
    def __init__(self, validation_ratio = 0.3, train_batch_size = 64, test_batch_size = 64, transform = None):
        ...
        self.train_dataset = datasets.CIFAR10(root = current_directory+"/CIFAR10", train = True, transform = self.custom_transformer, download = True)
        self.test_dataset = datasets.CIFAR10(root = current_directory+"/CIFAR10", train = False, transform = self.custom_transformer, download = True)
