...
class User:
    def __init__(self, in_channels: int = 1, num_classes: int = 10, num_boxes: int = 2, learning_rate: float = 1e-3, loader = None):
        print("USER: YOLOv1")
        print(f"DEVICE: {get_device_name()}")

        self.device = torch.device(get_device_name())
        self.model = YOLOv1(in_channels = in_channels, num_classes = num_classes, num_boxes = num_boxes).to(self.device)
